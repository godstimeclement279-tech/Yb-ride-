...
//
// Scope after the geocoding swap: this file now handles ONLY map tiles
// (token) + driving directions (route polyline + ETA). Place search and
// reverse geocoding moved to ./google.ts because Mapbox's OSM data has thin
// POI coverage for Agbor — the user confirmed real local landmarks
// returned "No matches found". Map tiles stay on Mapbox so we don't trigger
// a native rebuild (rnmapbox is already patched + shipped).
